...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.NexaRadarEngine=api})(globalThis,()=>{
  ...
  c('consciousness','Alteração de consciência',/alteracao (?:da |de )?consciencia|perda (?:da |de )?consciencia|inconscien\w*|glasgow\s*\d+|lucid\w*|confusao mental/,'exame_fisico','text');
  ...
  function facts(context={},observations=[]){
    ...
    for(const concept of Object.values(concepts)){
      ...
      for(const source of sources){
        ...
        for(const raw of source.text.replace(/[^.!?\n]*\?/g,'').split(/(?<!\d)[.;!?\n]+|\bmas\b|\bporem\b/i)){
          ...
          while((m=re.exec(text))){
            ...
            const unknown=/talvez|acho que|nao lembro|nao entendi|nao (?:sei|sabe|soube)|nao (?:informad|investigad|avaliad|perguntad)|desconhec/.test(text);
            const negative=/\b(?:nega|negou|sem|ausencia de|nao tenho|nao tive|nao sinto|nao houve|nunca tive)\b/.test(clause)||/^\s*(?::|=)?\s*(?:negad[oa]|ausente|nao\b)/.test(suffix)||/^(?:afebril|eupneic)/.test(m[0]);
            ...
            evidence.push({state:unknown?'unknown':negative?'known_absent':'known_present',temporal,section:source.section,quote:raw.trim()});
      ...
      out[concept.id]={state:last?.state||'not_asked',temporal:last?.temporal||'unknown',evidence,conflict,asked:turns.asked[concept.id]||null};
    }
    return out;
  }
  ...
  function analyze(context={},ledger={},aiItems=[],observations=[]){
    ...
    const relevantSex=norm(context.sex||fields.sexo||'');const age=Number(context.age||fields.idade);
    if(activated.some(p=>['abdomen','headache','urinary'].includes(p.id))&&(/feminino|mulher|^f$/.test(relevantSex)||/paciente.{0,12}(?:mulher|feminina)/.test(currentText))&&(!age||age>=10&&age<=55))add('pregnancy','high','Gestação ou puerpério podem modificar a avaliação.');
    ...
    for(const id of flagIds)if(active(id)&&!known[id].conflict&&concepts[id].type==='boolean')alerts.push({id:'finding:'+id,concept:id,priority:'critical',question:`Reavaliar o achado documentado: ${concepts[id].label.toLowerCase()}.`,evidence:known[id].evidence.filter(e=>e.state==='known_present'),reason:expected.get(id)?.reason||'Achado relevante no contexto atual; requer avaliação médica.',related:[id],kind:'documented_finding'});
  ...
  return {analyze,facts,answer,conversation,concepts,packs,norm,rank};
});
