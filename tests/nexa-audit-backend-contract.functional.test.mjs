...
for(const token of ["eq('snapshot_version',snapshotVersion)","validateDeidentifiedEnvelope","audit_schema_version:'3'","ALREADY_SUBMITTED"])assert.ok(edge.includes(token),`edge missing ${token}`);
for(const token of ['snapshot_version text not null','unique(source_consultation_id,snapshot_version)','for update','AUDIT_REVIEW_CONFLICT',"decision='discarded'",'delete from public.nexa_core_cases',"decision='corrected'","'{audit_corrected}'","'original_ai'","'physician_final'","'target'",'security definer','set search_path=public,pg_temp','grant execute on function public.submit_audit_review','revoke all on function public.get_audit_queue(text) from anon','grant execute on function public.get_audit_queue(text) to authenticated','revoke all on function public.get_core_dataset_summary() from anon','grant execute on function public.get_core_dataset_summary() to authenticated'])assert.ok(migration.toLowerCase().includes(token.toLowerCase()),`migration missing ${token}`);
assert.ok(migration.includes("eligible_for_learning',false"));
assert.ok(migration.includes("eligible_for_learning',true"));
