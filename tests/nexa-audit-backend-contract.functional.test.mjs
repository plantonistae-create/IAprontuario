...
const ctx=safeCoreContext({schema_version:'3',destination:{recommended:'alta',final:'internacao',status:'altered'},hypothesis_validation:{ai:'A',final:'B',cid:'Z00.0'},clinical_plan:{prescription:{items:[{name:'QA'}]}},protocol_usage:[{protocol_id:'p1',protocol_version_id:'v3'}],radar_learning:{items:[{concept:'bp',state:'known_present'}],answers:{bp:{documented:true}}},learning_layers:{original_ai:{hypothesis:'A'},physician_final:{hypothesis:'B'},audit_corrected:null},audit_submission_snapshot:{source_consultation_id:'11111111-1111-4111-8111-111111111111',source_sync_version:7,frontend_version:'18.10.1'}});
...
assert.equal(ctx.audit_submission_snapshot.source_sync_version,7);
assert.equal(ctx.radar_learning.items[0].concept,'bp');
assert.equal(ctx.radar_learning.answers.bp.documented,true);
assert.equal(ctx.protocol_usage[0].protocol_version_id,'v3');
assert.equal(ctx.clinical_plan.prescription.items[0].name,'QA');
