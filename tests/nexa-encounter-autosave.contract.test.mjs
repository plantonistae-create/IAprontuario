...
for(const token of ['indexedDB.open','encounter_id','ready_for_audit','consultation_history','upsert','nexa:encounter-local-saved','nexa:encounter-synced','audit_priority','pagehide','visibilitychange','online','offline'])assert.ok(code.includes(token),`missing ${token}`);
new Function(code);
  ...
  from(name){
    ...
    return{
      upsert(payload,{onConflict}){
        ...
        return{select(){return{single:async()=>({data:{id:payload.id,encounter_id:payload.encounter_id,encounter_state:payload.encounter_state,audit_priority:payload.audit_priority,audit_ready_at:payload.audit_ready_at,updated_at:new Date().toISOString(),sync_version:remote.length,status:payload.status},error:null})}}}
      }
    }
  }
};
