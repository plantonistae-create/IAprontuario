...
assert.ok(loader.includes('20260905-v1869'), 'loader precisa forçar cache-bust v18.6.9');
...
for (const forbidden of ['nexa-layout-static-v18.6.3.js','nexa-layout-static-v18.6.4.js','nexa-layout-flow-v18.6.js','nexa-gap-selfheal-v18.6.2.js','nexa-layout-flow-v18.5.js','nexa-layout-flow-v18.1.js','nexa-runtime-consolidated-v14.js','nexa-approved-ui-v15.js','nexa-parity-v17.js','nexa-parity-v17-hotfix.js']) assert.ok(!loader.includes(forbidden), `loader não pode carregar camada conflitante: ${forbidden}`);
...
for (const token of [
  '__NEXA_HISTORY_LIFECYCLE_V18_6_9__',
  ...
  'syncReviewToStyleExample(row.id,row.fields||{}',
  'styleExampleBackfill:true',
  'automaticClearAuditPreserved',
  'window.nexaRefreshSevenDayHistory'
...
assert.ok(sw.includes('nexa-v18-6-9-history-lifecycle-20260905'), 'service worker não está na versão v18.6.9');
assert.ok(sw.includes('20260905-v1869'), 'service worker precisa apontar para o hotfix v18.6.9');
...
console.log('NEXA UI + 7-day history + style examples + manual/automatic audit lifecycle + mobile shell contract: PASS');
