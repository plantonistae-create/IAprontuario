...
assert.ok(loader.includes('20260905-v1892'));
assert.ok(loader.lastIndexOf('nexa-auditor-panel-queue-v18.9.2.js')>loader.lastIndexOf('nexa-auditor-exact-v18.9.js'),'painel/fila deve carregar por último');
...
assert.ok(panelQueue.includes("typeof sb!=='undefined'"),'painel/fila deve reutilizar o cliente Supabase lexical do NEXA');
assert.ok(!panelQueue.includes('MutationObserver')&&!panelQueue.includes('setInterval'),'painel/fila não pode usar loops contínuos');
assert.ok(sw.includes('nexa-v18-9-2-panel-queue-20260905'));
assert.ok(sw.includes('20260905-v1892'));
assert.ok(sw.includes('cache:"no-store"'));
new Function(loader);new Function(layout);new Function(audit);new Function(draft);new Function(mobile);new Function(historyLifecycle);new Function(mobileFlow);new Function(exact);new Function(panelQueue);new Function(sw);
console.log('NEXA v18.9.2 auditor panel + queue focused slice: PASS');
