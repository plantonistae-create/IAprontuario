...
const layout = read('nexa-layout-static-v18.6.4.js');
...
for (const token of [
  '__NEXA_LAYOUT_STATIC_V18_6_4__',
  'nf1864StaticLayoutStyle',
  ...
  'recorderNormalized:',
  'recorderChildren:'
]) assert.ok(layout.includes(token), `layout v18.6.4 sem proteção esperada: ${token}`);

assert.ok(!layout.includes('MutationObserver'), 'layout v18.6.4 não pode usar MutationObserver e gerar loop visual');
assert.ok(!layout.includes('requestAnimationFrame'), 'layout v18.6.4 não pode recalcular deslocamento a cada frame');
assert.ok(!layout.includes('getBoundingClientRect'), 'layout v18.6.4 não deve depender de correção geométrica medida');
...
assert.ok(sw.includes('nexa-v18-6-4-recorder-grid-20260905'), 'service worker não está na versão v18.6.4');
...
console.log('NEXA UI + recorder grid root-cause fix + all-stage layout + audit contract: PASS');
